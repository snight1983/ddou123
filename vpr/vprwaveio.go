...
func delSilence(pnSrc []int16, K int) []int16 {
	var maxSampleValue int = -(SHRTMAX)
	var nSrcLen, outLength int64 = int64(len(pnSrc)), 0

	for i := int64(0); i < nSrcLen; i++ {
		if int(math.Abs(float64(pnSrc[i]))) > maxSampleValue {
			maxSampleValue = int(math.Abs(float64(pnSrc[i])))
		}
	}

	minVocEng := MINVOCENG
	if maxSampleValue < minVocEng {
		minVocEng = maxSampleValue / 2
	}

	if K > 50 {
		minVocEng = minVocEng + (maxSampleValue-minVocEng)*(K-50)/50
	} else if K < 50 {
		minVocEng = minVocEng - (minVocEng)*(50-K)/50
	}
	...
	var pnTarget []int16
	var pCur *[]int16
	var pWinBuf [VOCBLOCKLEN + 1]int16
	var nWin, nMod, i, k, eng int

	pnTarget = make([]int16, 0)
	pCur = &pnTarget

	nWin = int(nSrcLen) / VOCBLOCKLEN
	nMod = int(nSrcLen) % VOCBLOCKLEN

	for i = 0; i < nWin; i++ {
		eng = 0
		for k = 0; k < VOCBLOCKLEN; k++ {
			eng += int(math.Abs(float64(pnSrc[VOCBLOCKLEN*i+k])))
		}

		if eng > minVocEng*VOCBLOCKLEN {
			j, p = 0, 0
			old1, old2, old3 = 0, 0, 0
			for k = 0; k < VOCBLOCKLEN; k++ {
				curSample = pnSrc[VOCBLOCKLEN*i+k]
	...
	eng = 0
	for i = 0; i < nMod; i++ {
		eng += int(math.Abs(float64(pnSrc[VOCBLOCKLEN*nWin+i])))
	}

	if eng > minVocEng*nMod {
		j, p = 0, 0
		old1, old2, old3 = 0, 0, 0
		for i = 0; i < nMod; i++ {
			curSample = pnSrc[VOCBLOCKLEN*nWin+i]
			if curSample == old1 && old1 == old2 && old2 == old3 {
				if p >= 0 {
					j = p
				}
